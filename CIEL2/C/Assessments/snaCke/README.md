...
La valeur dite sentinelle `NULL` permet de détecter la fin de la liste chaînée.
...
> ℹ️ Le symbole ✍️ indique que la question nécessite une réponse rédigée (directement sur le document en lieu et place des balises `<br>`).
...
**1.1** 💻 - Sachant `IsKeyDown(KEY_UP)` permet de savoir quand la touche "flèche du haut" est appuyée adaptez les premières instructions de la fonction `update` pour permettre au serpent de descendre et d'aller à gauche.
...
**2.2.1** ✍️ - Comment se nomme la technique employée par la procédure `move_apple` pour déplacer la pomme sans nécessiter de valeur de retour et d'affectation par le code appelant ?
...
> ℹ️ - Pour rappel la fonction `check_square_collision` permet de détecter la collision entre deux carrés ... L'écran est un carré et chaque morceau du serpent est aussi un carré ...

> ℹ️ - La fonction `Vector2Zero()` vous permet d'obtenir un `Vector2` initialisé à (0, 0).

**4.2** 💻 - Complétez la fonction`free_snake` pour qu'elle libère la mémoire allouée par le serpent.

> ℹ️ - La tête du serpent est allouée sur la stack frame de la fonction main, il n'y a donc pas besoin de libérer cet espace mémoire.
...
> ℹ️ Avant de faire cette partie veillez à conserver une version fonctionnelle du jeu pour ne pas risquer de dégrader les autres parties de l'évaluation (un simple `cp main.c main.bak` devrait suffire).
